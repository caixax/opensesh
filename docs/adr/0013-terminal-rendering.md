...
- **New build step for shader authors.** Editing `shaders/` needs Qt's `qsb` (the Qt Shader Tools module: `aqt install-qt ... -m qtshadertools`, `qt6-shadertools` on Arch, `qt6-shader-baker` on Debian). Everyone else uses the committed `.qsb`. CI runs `cargo xtask shaders --check` with qsb 6.10.3. The vertex shader must never use input location 7, which the batchable rewrite takes.
