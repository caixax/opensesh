...
- **The CLI** (`opensesh-cli`, binary `opensesh`, no Qt): `list` reads `hosts.toml` and the linked sources itself; `connect` checks the host exists, `open` parses the target, then both send the request, or start the app next to the CLI (`OpenSesh.exe` in the Windows packages, `opensesh-app` elsewhere) with it when no instance answers. The packages install it (`/usr/bin/opensesh`, `opensesh.exe` next to the app on Windows).
...
- The Windows installer doesn't add the install folder to `PATH`; the README says how to use `opensesh.exe`.
